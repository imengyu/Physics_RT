...
MyPhantomOverlapListener::MyPhantomOverlapListener(sPhysicsWorld* world)
{
    this->world = world;
}
void MyPhantomOverlapListener::collidableAddedCallback(const hkpCollidableAddedEvent& event)
{
    ...
    if (world && world->physicsWorld && phantom && bodyOther) {
        ...
        sPhysicsBodyContactData data = { 0 };
        data.isRemoved = 1;
        world->callbacks.onPhantomOverlapCallback(phantom, bodyOther, phantom->id, bodyOther->id, true);
    }
}
void MyPhantomOverlapListener::collidableRemovedCallback(const hkpCollidableRemovedEvent& event)
{
    ...
    if (world && world->physicsWorld && phantom && bodyOther) {
        ...
        sPhysicsBodyContactData data = { 0 };
        data.isRemoved = 1;
        world->callbacks.onPhantomOverlapCallback(phantom, bodyOther, phantom->id, bodyOther->id, false);
    }
}
