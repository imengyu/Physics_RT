...
std::wstring Path::GetDirectoryName(std::wstring path)
{
	if (!StringHlp::StrEmeptyW(path.c_str())) {
		...
		while (--num >= 0)
		{
			wchar_t c = (path)[num];
			if (c == DirectorySeparatorChar || c == AltDirectorySeparatorChar || c == VolumeSeparatorChar)
				return std::wstring(path.substr(0, num));
		}
	}
	return std::wstring();
}
...
std::string Path::GetFileName(std::string path)
{
	if (!path.empty())
	{
		if (Path::CheckInvalidPathChars(path)) return path;
		size_t length = path.size();
		size_t num = length;
		while (--num >= 0)
		{
...
std::string Path::GetDirectoryName(std::string path)
{
	if (!path.empty())
	{
		...
		while (--num >= 0)
		{
			char c = (path)[num];
			if (c == DirectorySeparatorChar || c == AltDirectorySeparatorChar || c == VolumeSeparatorChar)
				return std::string(path.substr(0, num));
		}
	}
	return std::string();;
}
